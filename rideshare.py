...
def list_users():
	...
	src_dest_check=requests.post("http://52.70.63.132:80/api/v1/db/read",json={"insert":"","column":["name"],"table":"users","where":[]})
...
def clear_db():
	...
	res=requests.post("http://52.70.63.132:80/api/v1/db/write",json={"indicate":"3"})	
	if(res.json()==0):
...
def insert_rider():
	...
	read_res=requests.get("http://52.70.63.132:80/api/v1/users")
	if(read_res.json()==0):
